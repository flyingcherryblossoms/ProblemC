...
writeToExcel("Ques3", outputData=outputData)

Time = []
for indexi in range(len(minDistances)):
    Time.append(cal_time(minDistances[indexi], taskLists3[indexi]))
print("出库时间", sum(Time), 's')
...
for indexi in range(len(RS)):
    ...
    for indexj in range(len(minPaths)):
        if minPaths[indexj][-1]-len(minPaths[indexj])+2 == RS[indexi]:
            Timei[indexi].append(Time[indexj])
for indexi in range(len(Timei)):
    if len(Timei[indexi]) != 0:
        print(str(RSName[indexi]) + "利用率：", float(len(Timei[indexi])*30)/(len(Timei[indexi])*30+sum(Timei[indexi])))
    else:
