...
def readExcel(filename):
    # 读取Excel
    file = pd.ExcelFile("../P4/src/" + filename + ".xlsx")
    # 任务单T0001
    data = []
...
RSP = [0, 2, 9, 11]  # 货单编号
RSO = [1, 3, 4, 5, 6, 7, 8, 10, 12]
RSPName = ["FH01", "FH03", "FH10", "FH12"]
RSOName = ["FH02", "FH04", "FH05", "FH06", "FH07", "FH08", "FH9", "FH11", "FH13"]
for indexO in range(len(RSO)):
    RS = RSP.copy()
    RS.append(RSO[indexO])
    RSName = RSPName.copy()
    RSName.append(RSOName[indexO])
    # print(RS)
    # print(RSName)
    ...
    origins = RS.copy()  # 记录每一个点的起点，初始起点为FH03
    ...
    for indexi in range(len(taskLists)):
        ...
        for indexj in range(len(origins)):
            ...
            minDistancej, minPathj, ccPathj, minRegisterj = GA.run(1, RS)
            # print(minDistancej)
    ...
    for indexi in range(len(taskLists)):
        for indexj in minPaths[indexi]:
            for indexk in range(len(PTime)):
                if taskLists[indexi][indexj][0] in PTime[indexk][0] or indexj == minPaths[indexi][0] or indexj == \
                        minPaths[indexi][-1]:
    ...
    #     print(outputData[i])
    writeToExcel("Ques5_"+str(indexO), outputData=outputData)
