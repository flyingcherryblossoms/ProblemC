...
class GATSP:
    ...
    def run(self, times=1, RS=None):
        # print(self.taskList[self.origin][2])
        ...
        for timei in range(times):
            ...
            for rs in RS:  # 去掉最后一个点（终点，或者起点），这里由于要满足起点和终点的要求，不能倒置
                ccPath = []  # 记录货格名称
                tempPath.pop()  # 去尾
                tempPath.append(rs + self.point_count)  # 加复核台
                pathlenth = self.get_distance(tempPath)
                if self.minDistance > pathlenth:
                    self.minDistance = pathlenth
